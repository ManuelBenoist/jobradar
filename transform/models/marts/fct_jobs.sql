{{ config(
    materialized='table',
    description='Moteur de scoring Hybride V3.0 - Intégration de règles métier et IA Sémantique',
    external_location="s3://jobradar-curated-manuel-cloud/gold/fct_jobs/"
) }}

WITH 

-- 1. PRÉPARATION DES RÉFÉRENTIELS
profile AS (
    /* Récupération du vecteur d'embedding du profil idéal généré en amont */
...
vector_math AS (
    /* 
       Calcul vectoriel pur en SQL (Athena/Presto). 
       Calcule le produit scalaire normalisé entre l'offre et le profil idéal.
    */
    SELECT
        j.job_id,
...
-- 3. MOTEUR DE RÈGLES : ANALYSE DES TEXTES (REGEX)
text_processing AS (
    SELECT 
        *, 
        LOWER(title) as l_title, 
        LOWER(description) as l_desc 
    FROM jobs_base
),
...
scoring_logic AS (
    SELECT
        ...
        CASE 
            WHEN regexp_like(l_title, '\b(stage|internship|alternance|apprentissage)\b') 
              OR (regexp_like(l_desc, '\bgratification\b') AND regexp_like(l_desc, '\bstagiaire\b')) 
            THEN {{ var('penalty_internship') }} 
            ELSE 0 
        END as score_veto_intern,
        ...
        -- --- BLOC ÉTHIQUE & IMPACT (Gestion fine des faux-positifs) ---
        CASE 
            ...
            WHEN (
                ...
                OR regexp_like(l_desc, '\bcomptable\b') 
                OR (regexp_like(l_desc, '\bassurance\b') AND NOT regexp_like(l_desc, '\bassurance\s+(de|que|d''être|d''avoir)\b'))
        ...
        CASE 
            WHEN regexp_like(l_desc, 'b corp|b-corp|société à mission|économie sociale') THEN {{ var('bonus_ethics') }} 
            WHEN regexp_like(l_desc, 'décarbonation|écologie|climat') THEN 15 
            ELSE 0 
        END as score_impact,
...
-- 4. AGRÉGATION DES PILIERS DE SCORE
pillars AS (
    SELECT
        j.job_id,
    ...
    FROM jobs_base j
    LEFT JOIN scoring_logic s ON j.job_id = s.job_id
),

-- 5. GÉNÉRATION DES LABELS UI (USER EXPERIENCE)
labels_extraction AS (
    SELECT
        ...
        ARRAY_JOIN(FILTER(ARRAY[
            ...
            CASE WHEN p_tech_skills > 40 THEN '🚀 Stack intéressante' END
        ], x -> x IS NOT NULL), ' | ') AS positive_labels,

        ARRAY_JOIN(FILTER(ARRAY[
            CASE WHEN p_internship_veto < 0 THEN '🚫 Stage/Alternance' END,
            CASE WHEN p_exp_penalty <= -50 THEN '⚠️ Offre pour séniors' END,
...
final_calculation AS (
    SELECT
        ...
        (
            {{ var('base_score') }} + 
            COALESCE(p.p_exp_bonus, 0) + COALESCE(p.p_exp_penalty, 0) + 
            COALESCE(p.p_internship_veto, 0) + COALESCE(p.p_years_veto, 0) + 
            COALESCE(p.p_status_penalty, 0) + COALESCE(p.p_ethics, 0) + 
            COALESCE(p.p_impact_positive, 0) + COALESCE(p.p_culture_training, 0) + 
            COALESCE(p.p_culture_os, 0) + COALESCE(p.p_tech_skills, 0) + 
            COALESCE(p.p_remote, 0) + COALESCE(p.p_freshness, 0)
        ) AS raw_rule_score,
...
SELECT 
    *,
    -- Normalisation des scores sur une échelle de 0 à 100
    CASE 
        WHEN raw_rule_score > 100 THEN 100 
        WHEN raw_rule_score < 0 THEN 0 
        ELSE raw_rule_score 
    END AS rules_score,
    
    CASE 
        WHEN raw_nlp_score > 100 THEN 100 
        WHEN raw_nlp_score < 0 THEN 0 
