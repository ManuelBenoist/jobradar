...
st.caption("Veille automatisée de recherche d'emplois Data & DevOps | Pipeline ELT Serverless sur AWS")
...
with tab_radar:
    if data and "jobs" in data and len(data["jobs"]) > 0:
        ...
        # NETTOYAGE & CALCULS
        df['matching_score'] = pd.to_numeric(df['matching_score'], errors='coerce').fillna(0).astype(int)
        ...
        k4.metric("Salaire Moyen", f"{int(df['salary_min'].mean()):,} €".replace(',', ' '))
        ...
        st.warning("Aucune donnée disponible. La Lambda est peut-être en cours de réveil.")
